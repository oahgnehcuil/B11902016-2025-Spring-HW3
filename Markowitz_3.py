...
class MyPortfolio:
    ...
    def calculate_weights(self):
        ...
        for i in range(self.lookback + 1, len(df)):
            R_n = df.copy()[assets].iloc[i - self.lookback : i]
    ...
    def mv_opt(self, R_n, gamma):
        ...
        with gp.Env(empty=True) as env:
            ...
            with gp.Model(env=env) as model:
                ...
                model.setObjective(lin - 4 * quad, gp.GRB.MAXIMIZE)
...
class AssignmentJudge:
    ...
    def check_sharp_ratio_greater_than_spy(self):
        '''
        onne = self.report_metrics(Bdf, self.Bmp)[1]
        csed = self.report_metrics(Bdf, self.Bmp)[0]
        print(onne,csed)
        '''
        if not self.check_portfolio_position(self.mp[0]):
            return 0
